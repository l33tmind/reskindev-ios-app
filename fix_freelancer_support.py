...
with open('next-frontend/src/app/freelancer/page.js', 'r') as f:
    content = f.read()

# 1. Add Support Modal State
old_state = """  const [completionImage, setCompletionImage] = useState(null);
  const [isUploading, setIsUploading] = useState(false);"""
new_state = """  const [completionImage, setCompletionImage] = useState(null);
  const [isUploading, setIsUploading] = useState(false);
...
support_func = """  const submitSupport = async () => {
    ...
    try {
      await addDoc(collection(db, "support_tickets"), {
        ...
        reportedBy: user.uid,
        targetId: supportOrder.clientUid,
...
if "const submitSupport =" not in content:
    content = content.replace("  const handleCompleteOrder = async () => {", support_func + "const handleCompleteOrder = async () => {")

# 3. Add Contact Support button
old_btn = """                  <ContactUserButton 
                    freelancerId={order.clientUid} 
...
if "{/* Support Modal */}" not in content:
    content = content.replace("      {/* Complete Order Modal */}", modal_ui + "\n      {/* Complete Order Modal */}")

with open('next-frontend/src/app/freelancer/page.js', 'w') as f:
    f.write(content)
print("Updated freelancer page with support")
