...
# The block to move
media_block = """                    _buildSectionTitle('Media'),
                    Column(
...
if media_block in content:
    # Remove from old position
    content = content.replace(media_block, "")
    
    # Insert before PACKAGE TIERS
    target = "// ── PACKAGE TIERS (TABS) ──────────────────────────"
    content = content.replace(target, media_block + "\n\n                    " + target)
    
    with open('lib/screens/gig_editor_screen.dart', 'w') as f:
        f.write(content)
    print("Moved media block to the correct place")
else:
